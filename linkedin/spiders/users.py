    ...
    def parse_cat(self, response):
        ...
        for link in links:
            ...
            if not link.startswith('http'):
                ...
                l = link
            yield scrapy.Request(l, callback=self.parse_city_cat)

    def parse_city_cat(self, response):
        item = items.LinkedinUserItem()
        item['member-connections'] = response.css('.member-connections>strong::text').extract()
        item['page_url'] = response.url
        item['full_name'] = "".join(response.css('#name::text').extract()).strip()
        present_expirience = response.xpath('.//*[@id="experience"]/ul/li[@class="position"and descendant::span[not(text()="")]]')
